...
if open_position:
    if position['side'] == 'long':
        close_side = 'sell'
        stop_loss_price = float(position['info']['avgPrice']) * (1 - params['stop_loss_pct'])
...
usdt_balance = float(current_balance['total'].get('USDT', 0)) / 1000 # Ensure it's a float, default to 0 if missing
